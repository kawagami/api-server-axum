...
個人網站的全端 monorepo:**Rust / Axum 後端** + **Next.js 前端**,合併於單一 repo。
部署編排(docker-compose / nginx / certbot)獨立在 `docker-env` repo。
...
frontend/ ──CI build──▶ kawagami77/my-next-blog:latest ─┤
                                                         ▼
docker-env(獨立 repo)── compose + nginx + certbot + .env ──▶ Vultr VPS
```

1. **Docker 映像**:前後端各自 build image 推到 Docker Hub;image 名稱是契約,改名要同步 docker-env。
2. **HTTP / WebSocket**:前端用 `API_URL` / `WS_URL` 環境變數連後端,URL 不寫死。
3. **編排**:docker-env 的 compose + nginx vhost(`kawa.homes` → 前端、`axum.kawa.homes` → 後端)。
...
- **docker-env** — 部署編排(compose / nginx / certbot / 部署腳本)。
