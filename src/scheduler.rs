...
pub async fn initialize_scheduler(state: AppState) {
    ...
    add_job_if_enabled(&scheduler, state.clone(), CleanupUnusedImagesJob).await;
    add_job_if_enabled(&scheduler, state.clone(), FetchStockDayAllJob).await;
    add_job_if_enabled(&scheduler, state.clone(), FetchBuybackPeriodsJob).await;
    add_job_if_enabled(&scheduler, state.clone(), FetchNotesJob).await;
    add_job_if_enabled(&scheduler, state.clone(), FetchHistoricalClosingPricesJob).await;
    add_job_if_enabled(&scheduler, state.clone(), ConsumePendingStockChangeJob).await;
    add_job_if_enabled(&scheduler, state.clone(), SyncBuybackToPendingJob).await;

    scheduler.start().await.expect("failed to start scheduler");
}

async fn add_job_if_enabled<J: AppJob + Send + Sync + 'static>(
