...
async fn get_oauth_url(
    ...
    let url = oauth_service::generate_oauth_url(state.get_redis_pool(), state.get_config(), &settings, &provider).await?;
...
async fn exchange_code(
    ...
    let settings = state.get_settings();
    let (access_token, refresh_token) = oauth_service::exchange_code(
        state.get_pool(),
...
async fn refresh_token(
    ...
    let (access_token, refresh_token) =
        oauth_service::refresh_member_token(state.get_redis_pool(), state.get_config(), &payload.refresh_token).await?;
