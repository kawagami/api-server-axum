...
- ~~`games/common/room.rs` 缺 `broadcast_to_room`、N 人房 `err1` 三份、2 人框架 inline error flush 11 處、avalon/farm `handle_disconnect` 純轉發、`start_game` 骨架逐行對應~~（2026-09-25 收斂：`room.rs` 新增 `broadcast_to_room`（吃掉 avalon 6 + farm 1 + `push_room_update` 1 處迴圈）與 `start_game(hub, state, who, setup, on_started)`（avalon 只剩 `announce_start` 推 `role_assigned` → `phase_changed`，farm 直接傳 `broadcast_state`）；兩支轉發廢函式刪掉，`registry.rs` 直呼 `room::handle_disconnect`；`common/service.rs` 補私有 `err1::<E>`，11 處 inline 全換掉（`send_to` 與單則 `flush` 同樣走 `send_many`，行為不變）。實際淨減只有約 10 行 —— **原估的 150–170 行高估了**：avalon/farm 的對局動作各自不同，同構的只有開局骨架與廣播迴圈）。**刻意留著的**：avalon / farm 私有的 `msg` / `err1` 是一行委派 `room::msg::<K>` / `room::err1::<K>` 的 turbofish 縮寫，不是重複邏輯；2 人框架的 `illegal_move` 兩處 inline（型別不是 `error`）。~~`flush` 逐字重複兩份~~（2026-08-06 兩邊都改成委派 `AppState::send_outbox`，剩一行）。
